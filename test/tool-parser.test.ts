...
import type { EarlyExecutableToolCall, NormalizedToolCall } from "../src/types.js"
import {
	anthropicChunks,
	deepseekChunks,
	geminiChunks,
...
describe("ToolStream", () => {
	...
	it("should parse OpenAI single streaming tool call", () => {
		...
		parser.on("toolStarted", (_id, name) => events.push(`toolStarted:${name}`))
	...
	it("should emit events in correct order", () => {
		...
		parser.on("toolStarted", (_id, name) => eventLog.push(`toolStarted:${name}`))
	...
	it("should emit early-execute event when confidence threshold met", () => {
		...
		const _earlyCalls: EarlyExecutableToolCall[] = []
		parser.on("toolCompleted", (_tc) => {
	...
	it("should emit token events during parsing", () => {
		...
		parser.push(
			`{"choices":[{"delta":{"tool_calls":[{"index":0,"id":"call_t1","function":{"name":"fn","arguments":""}}]}}]}`,
		)
		parser.finalize()
