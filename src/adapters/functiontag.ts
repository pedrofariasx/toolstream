...
export class FunctionTagAdapter extends BaseAdapter {
  ...
  detect(chunk: string): boolean {
    return chunk.includes('<tool_call>') ||
  ...
  private handleTag(tag: string): NormalizedDelta[] | null {
    ...
    if (content.startsWith('function=')) {
      if (!isClosing) {
        ...
        return [
          this.createToolDetected(this.current.index),
          this.createToolName(name, this.current.index),
        ]
      }
      return null
    }
  ...
  private flushParameter(): void {
    if (this.current && this.current.currentParamKey) {
  ...
  private finishToolCall(): NormalizedDelta[] | null {
    if (!this.current || !this.current.name) return null
