import type { PartialJsonResult, RepairSuggestion, RepairChange } from './types.js'
...
export class JsonStreamParser {
  ...
  private processChar(char: string): void {
    switch (this.state) {
      case JsonStreamState.VALUE: this.handleValue(char); break
      case JsonStreamState.STRING: this.handleString(char); break
      case JsonStreamState.STRING_ESCAPE: this.handleStringEscape(char); break
      case JsonStreamState.NUMBER: this.handleNumber(char); break
      case JsonStreamState.NUMBER_FRACTION: this.handleNumberFraction(char); break
      case JsonStreamState.NUMBER_EXP: this.handleNumberExp(char); break
      case JsonStreamState.NUMBER_EXP_VALUE: this.handleNumberExpValue(char); break
      case JsonStreamState.TRUE_1: this.state = char === 'r' ? JsonStreamState.TRUE_2 : JsonStreamState.ERROR; break
      case JsonStreamState.TRUE_2: this.state = char === 'u' ? JsonStreamState.TRUE_3 : JsonStreamState.ERROR; break
      case JsonStreamState.TRUE_3: this.state = char === 'e' ? this.afterScalar() : JsonStreamState.ERROR; break
      case JsonStreamState.FALSE_1: this.state = char === 'a' ? JsonStreamState.FALSE_2 : JsonStreamState.ERROR; break
      case JsonStreamState.FALSE_2: this.state = char === 'l' ? JsonStreamState.FALSE_3 : JsonStreamState.ERROR; break
      case JsonStreamState.FALSE_3: this.state = char === 's' ? JsonStreamState.FALSE_4 : JsonStreamState.ERROR; break
      case JsonStreamState.FALSE_4: this.state = char === 'e' ? this.afterScalar() : JsonStreamState.ERROR; break
      case JsonStreamState.NULL_1: this.state = char === 'u' ? JsonStreamState.NULL_2 : JsonStreamState.ERROR; break
      case JsonStreamState.NULL_2: this.state = char === 'l' ? JsonStreamState.NULL_3 : JsonStreamState.ERROR; break
      case JsonStreamState.NULL_3: this.state = char === 'l' ? this.afterScalar() : JsonStreamState.ERROR; break
      case JsonStreamState.OBJECT_KEY: this.handleObjectKey(char); break
      case JsonStreamState.OBJECT_COLON: this.handleObjectColon(char); break
      case JsonStreamState.OBJECT_VALUE: this.handleObjectValueAfter(char); break
      case JsonStreamState.OBJECT_COMMA: this.handleObjectComma(char); break
      case JsonStreamState.ARRAY_VALUE: this.handleArrayValueStart(char); break
      case JsonStreamState.ARRAY_COMMA: this.handleArrayComma(char); break
    }
  }
  ...
  private handleValue(char: string): void {
    ...
    switch (char) {
      ...
      case '"':
        ...
        break
      case 't': this.state = JsonStreamState.TRUE_1; break
      case 'f': this.state = JsonStreamState.FALSE_1; break
      case 'n': this.state = JsonStreamState.NULL_1; break
      case '-':
        this.currentNumber = '-'
  ...
  private handleStringEscape(char: string): void {
    switch (char) {
      case '"': this.currentString += '"'; break
      case '\\': this.currentString += '\\'; break
      case '/': this.currentString += '/'; break
      case 'b': this.currentString += '\b'; break
      case 'f': this.currentString += '\f'; break
      case 'n': this.currentString += '\n'; break
      case 'r': this.currentString += '\r'; break
      case 't': this.currentString += '\t'; break
      case 'u':
        this.currentString += '\\u'
        break
      default: this.currentString += char
    }
    this.state = JsonStreamState.STRING
  }
  ...
  fixPartial(): string {
    ...
    if (this.state === JsonStreamState.NUMBER ||
        this.state === JsonStreamState.NUMBER_FRACTION ||
        this.state === JsonStreamState.NUMBER_EXP ||
        this.state === JsonStreamState.NUMBER_EXP_VALUE) {
      if (this.currentNumber.endsWith('.') || this.currentNumber.endsWith('e') ||
          this.currentNumber.endsWith('E') || this.currentNumber.endsWith('+') ||
...
export class JsonRepairer {
  static repair(input: string): RepairSuggestion | null {
    ...
    const needsQuote = this.needsClosingQuote(repaired)
    if (needsQuote) {
      repaired += '"'
      changes.push({ type: 'insert', offset: repaired.length - 1, original: '', replacement: '"' })
    }

    const closings = this.computeMissingClosings(repaired)
    ...
    repaired = this.fixTrailingColon(repaired)
    repaired = this.fixUnquotedKeys(repaired)
    repaired = this.fixSingleQuotes(repaired)
    repaired = this.fixTrailingDecimal(repaired)
    repaired = this.fixIncompleteBeforeBrace(repaired)
    ...
    return {
      ...
      repaired,
      changes,
      confidence: this.calculateConfidence(changes.length),
    }
  }

  private static needsClosingQuote(s: string): boolean {
    const inString = this.isInsideUnclosedString(s)
    return inString
  }
