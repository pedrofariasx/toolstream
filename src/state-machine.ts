...
import { type PendingToolCall, StateMachineState } from "./types.js"
...
export class ToolCallStateMachine {
	...
	private processChar(char: string): void {
		switch (this.state) {
			case StateMachineState.Idle:
				this.processIdle(char)
				break
			case StateMachineState.Text:
				this.processText(char)
				break
			case StateMachineState.PossibleTool:
				this.processPossibleTool(char)
				break
			case StateMachineState.ToolName:
				this.processToolName(char)
				break
			case StateMachineState.Arguments:
				this.processArguments(char)
				break
			case StateMachineState.Json:
				this.processJson(char)
				break
			case StateMachineState.Escape:
				this.processEscape(char)
				break
			case StateMachineState.Completed:
				this.processCompleted(char)
				break
	...
	private processPossibleTool(char: string): void {
		...
		if (
			ctx.includes("tool_calls") ||
			ctx.includes("function") ||
			ctx.includes("tool_use") ||
			ctx.includes("functionCall") ||
